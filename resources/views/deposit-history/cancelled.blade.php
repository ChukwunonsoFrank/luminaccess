<x-app-layout>
    <h1>Cancelled deposits history</h1>
    <p><b>Total cancelled fundings</b>: ${{ $total_cancelled }}</p>
<br><br>
    <div class="card">
        <div class="card-header">
            <h4>View your cancelled funding history below</h4>
        </div>
        <div class="card-body">
                        ...
                        <div class="col-sm-12">
                            <table class="table table-striped dataTable no-footer" id="table-2" role="grid"
                                ...
                                <tbody>
                                    ...
                                    @foreach($cancelled_deposits as $deposit)
                                    <tr role="row" class="odd">

                                        <td class="sorting_1">{{ $deposit->created_at }}</td>
                                        <td>{{ $deposit->amount }}</td>
                                        <td>{{ $deposit->payment_method }}</td>

                                        <td>
                                            <div class="badge badge-warning">{{ $deposit->confirmation_status }}</div>
                                        </td>
